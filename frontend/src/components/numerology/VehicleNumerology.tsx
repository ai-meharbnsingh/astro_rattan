...
export default function VehicleNumerology({ birthDate }: Props) {
  ...
  const analyzeVehicle = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Vehicle numerology analysis failed. Please try again.');
    }
    setLoading(false);
  };
        ...
        <CardContent className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                placeholder="e.g., MH 01 AB 1234 or DL4CAX1234"
                value={vehicleNumber}
                onChange={(e) => setVehicleNumber(e.target.value)}
                className="bg-cosmic-card border-sacred-gold"
              />
              <p className="text-xs text-cosmic-text-secondary mt-1">
                Enter full registration number - letters and digits will be analyzed
              </p>
            </div>
            ...
            <div>
              <label className="block text-sm text-cosmic-text-secondary mb-1">
                Owner's Date of Birth (Optional - for compatibility)
              </label>
              <Input
                type="date"
                value={dob}
            ...
            >
              {loading ? (
                ...
                <><Car className="w-4 h-4 mr-2" />Analyze Vehicle Number</>
              )}
            </Button>
          </div>
        </CardContent>
      ...
      {result && (
        <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4 text-center">
                <p className="text-xs text-cosmic-text-secondary mb-1">Vehicle Vibration</p>
            ...
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4 text-center">
                <p className="text-xs text-cosmic-text-secondary mb-1">Digit Sum</p>
                <Badge className="text-xl px-3 py-1 bg-blue-100 text-blue-700">{result.vibration.digit_sum}</Badge>
                <p className="text-xs text-cosmic-text-secondary mt-2">Before reduction</p>
              </CardContent>
            </Card>
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4 text-center">
                <p className="text-xs text-cosmic-text-secondary mb-1">Letter Value</p>
                <Badge className="text-xl px-3 py-1 bg-purple-100 text-purple-700">{result.vibration.letter_value}</Badge>
                <p className="text-xs text-cosmic-text-secondary mt-2">From registration</p>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4 pt-3 border-t border-sacred-gold/20">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-cosmic-text">Driving Style</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-cosmic-text">Caution</p>
            ...
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-sm font-medium text-cosmic-text">Lucky Colors</p>
                </div>
            ...
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-sm font-medium text-cosmic-text">Lucky Directions</p>
                </div>
                <div className="flex flex-wrap gap-2">
          ...
          {result.digit_analysis.length > 0 && (
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4">
                <p className="text-sm font-medium text-cosmic-text mb-3">Digit-by-Digit Analysis</p>
          ...
          {result.special_combinations.length > 0 && (
            <Card className="bg-cosmic-card border-0 shadow-soft">
              <CardContent className="p-4">
                <p className="text-sm font-medium text-cosmic-text mb-3">Special Number Patterns</p>
                <div className="space-y-2">
                  {result.special_combinations.map((combo, i) => (
                    <div key={i} className="flex items-center gap-3 p-2 bg-sacred-gold/5 rounded-lg">
                      ...
                      <span className="text-sm text-cosmic-text-secondary">{combo.meaning}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card className={`border-0 shadow-soft ${result.owner_compatibility.is_favorable ? 'bg-green-50/50' : 'bg-yellow-50/50'}`}>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-sm font-medium text-cosmic-text">Owner Compatibility</p>
                </div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-cosmic-text-secondary">Your Life Path: {result.owner_compatibility.owner_life_path}</span>
                  <span className="text-sm text-cosmic-text-secondary">Vehicle: {result.owner_compatibility.vehicle_number}</span>
                </div>
                <Badge className={result.owner_compatibility.is_favorable 
                  ? 'bg-green-100 text-green-800' 
                  : 'bg-yellow-100 text-yellow-800'
                }>
                  {result.owner_compatibility.is_favorable ? '✓ Favorable Match' : '⚠ Neutral/Challenging'}
                </Badge>
