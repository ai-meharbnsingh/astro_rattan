    ...
    <div className="fixed bottom-6 right-6 z-40 flex items-end gap-3 ai-chat-hide">
      {showTooltip && (
        <div className="relative bg-cosmic-card border border-sacred-gold/20 rounded-2xl rounded-br-sm px-4 py-3 shadow-lg max-w-[200px] animate-fade-in">
          <button onClick={() => setShowTooltip(false)} className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-cosmic-surface border border-sacred-gold/20 flex items-center justify-center text-cosmic-text/70 hover:text-cosmic-text">
            <X className="w-3 h-3" />
          </button>
