...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-xl border border-sacred-gold/20 bg-card overflow-hidden"
    >
      <table
        data-slot="table"
        className={cn("table-sacred w-full caption-bottom text-sm", className)}
        {...props}
      />
    </div>
  )
