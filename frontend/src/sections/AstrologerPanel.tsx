  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Button variant="outline" size="sm" onClick={onCancel} className="border-[#B8860B]/30 text-[#8B7355]">
          <ArrowLeft className="w-4 h-4 mr-1" />{t('astrologer.backToClients')}
        </Button>
        <h3 className="text-xl font-display font-bold text-[#1a1a2e]">{t('astrologer.addClient')}</h3>
      </div>
      ...
      <div className="space-y-4">
        {/* Name */}
        <div>
          <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">{t('astrologer.clientName')} *</label>
          <div className="relative">
            <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#B8860B]" />
            <Input value={form.client_name} onChange={(e) => setForm({ ...form, client_name: e.target.value })}
              placeholder="Full Name" className="pl-10 bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e]" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">{t('astrologer.clientPhone')}</label>
            <div className="relative">
              <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#B8860B]" />
              <Input value={form.client_phone} onChange={(e) => setForm({ ...form, client_phone: e.target.value })}
                placeholder="+91..." className="pl-10 bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e]" />
            </div>
          </div>
          <div>
            <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">{t('astrologer.clientEmail')}</label>
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#B8860B]" />
              <Input type="email" value={form.client_email} onChange={(e) => setForm({ ...form, client_email: e.target.value })}
                placeholder="email@example.com" className="pl-10 bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e]" />
            </div>
          </div>
        </div>
        ...
        {/* Gender */}
        <div>
          <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Gender</label>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setForm({ ...form, gender: 'male' })}
              className={`p-2.5 border text-sm transition-colors ${form.gender === 'male' ? 'border-[#B8860B] bg-[#B8860B]/10 text-[#B8860B]' : 'border-[#B8860B]/15 text-[#8B7355]'}`}>
              Male
            </button>
            <button onClick={() => setForm({ ...form, gender: 'female' })}
              className={`p-2.5 border text-sm transition-colors ${form.gender === 'female' ? 'border-[#B8860B] bg-[#B8860B]/10 text-[#B8860B]' : 'border-[#B8860B]/15 text-[#8B7355]'}`}>
              Female
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Birth Date</label>
            <div className="relative">
              <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#B8860B]" />
              <Input type="date" value={form.birth_date} onChange={(e) => setForm({ ...form, birth_date: e.target.value })}
                className="pl-10 bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e]" />
            </div>
          </div>
          <div>
            <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Birth Time</label>
            <Input type="time" step="1" value={form.birth_time} onChange={(e) => setForm({ ...form, birth_time: e.target.value })}
              className="bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e]" />
          </div>
        </div>

        {/* Birth Place with geocode */}
        <div ref={placeRef} className="relative">
          <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">Birth Place</label>
          <div className="relative">
            ...
            <Input value={form.birth_place}
              ...
              placeholder="City, Country" className="pl-10 bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e]" />
          ...
          {geocode.showDropdown && geocode.suggestions.length > 0 && (
            <div className="absolute z-20 left-0 right-0 mt-1 bg-[#F5F0E8] border border-[#B8860B]/20 shadow-lg max-h-48 overflow-y-auto">
              {geocode.suggestions.map((s, i) => (
                <button key={i} onClick={() => {
                  setForm({ ...form, birth_place: s.name.split(',').slice(0, 2).join(','), latitude: s.lat, longitude: s.lon });
                  geocode.close();
                }} className="block w-full text-left px-3 py-2 text-sm text-[#1a1a2e] hover:bg-[#B8860B]/10 truncate">
                  {s.name}
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="text-xs font-medium text-[#8B7355] mb-1 block">Latitude</label>
            <Input type="number" step="0.0001" value={form.latitude}
              onChange={(e) => setForm({ ...form, latitude: parseFloat(e.target.value) || 0 })}
              className="bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e] text-sm" />
          </div>
          <div>
            <label className="text-xs font-medium text-[#8B7355] mb-1 block">Longitude</label>
            <Input type="number" step="0.0001" value={form.longitude}
              onChange={(e) => setForm({ ...form, longitude: parseFloat(e.target.value) || 0 })}
              className="bg-[#F5F0E8] border-[#B8860B]/15 text-[#1a1a2e] text-sm" />
          </div>
          <div>
            <label className="text-xs font-medium text-[#8B7355] mb-1 block">TZ Offset</label>
        ...
        {/* Notes */}
        <div>
          <label className="text-sm font-medium text-[#1a1a2e] mb-1 block">{t('astrologer.notes')}</label>
...
function ClientKundliView({ client, onBack, onNotesUpdate }: ClientKundliViewProps) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        <Button variant="outline" size="sm" onClick={onBack} className="border-[#B8860B]/30 text-[#8B7355]">
          <ArrowLeft className="w-4 h-4 mr-1" />{t('astrologer.backToClients')}
        </Button>
      </div>

      {/* Client Info Card */}
      <Card className="bg-[#E8E0D4] border-0 shadow-sm mb-6">
        <CardContent className="p-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-xl font-display font-bold text-[#1a1a2e]">{client.client_name}</h3>
              <div className="flex flex-wrap gap-3 mt-1 text-sm text-[#8B7355]">
                {client.client_phone && (
      ...
      <Card className="bg-[#E8E0D4] border-0 shadow-sm mb-6">
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-2">
            <h4 className="font-medium text-[#1a1a2e]">{t('astrologer.notes')}</h4>
            {!editingNotes ? (
              <Button variant="outline" size="sm" onClick={() => setEditingNotes(true)}
                className="border-[#B8860B]/30 text-[#8B7355] text-xs">
                Edit
              </Button>
            ) : (
          ...
          {editingNotes ? (
            ...
            <p className="text-sm text-[#8B7355]">{client.notes || 'No notes yet.'}</p>
          )}
        </CardContent>
      </Card>
      ...
      {loadingKundli ? (
        ...
        <div>
          <h4 className="text-lg font-display font-semibold text-[#1a1a2e] mb-4">{t('astrologer.viewKundli')}</h4>
          <InteractiveKundli
            chartData={{ planets }}
            onPlanetClick={() => {}}
            onHouseClick={() => {}}
          ...
          {planets.length > 0 && (
            <Card className="bg-[#E8E0D4] border-0 shadow-sm mt-6">
              <CardContent className="p-5">
                <h4 className="font-medium text-[#1a1a2e] mb-3">Planet Positions</h4>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#B8860B]/20">
                        <th className="text-left py-2 px-2 text-[#8B7355] font-medium">Planet</th>
                        <th className="text-left py-2 px-2 text-[#8B7355] font-medium">Sign</th>
                        <th className="text-left py-2 px-2 text-[#8B7355] font-medium">House</th>
                        <th className="text-left py-2 px-2 text-[#8B7355] font-medium">Degree</th>
                        <th className="text-left py-2 px-2 text-[#8B7355] font-medium">Nakshatra</th>
                      </tr>
                    </thead>
                    <tbody>
                      {planets.map((p) => (
                        <tr key={p.planet} className="border-b border-[#B8860B]/10">
                          <td className="py-2 px-2 text-[#1a1a2e] font-medium">{p.planet}{p.status === 'Retrograde' ? ' (R)' : ''}</td>
                          <td className="py-2 px-2 text-[#1a1a2e]">{p.sign}</td>
                          <td className="py-2 px-2 text-[#1a1a2e]">{p.house}</td>
                          <td className="py-2 px-2 text-[#1a1a2e]">{typeof p.sign_degree === 'number' ? p.sign_degree.toFixed(2) : p.sign_degree}</td>
                          <td className="py-2 px-2 text-[#1a1a2e]">{p.nakshatra}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="text-center py-12">
          <Calendar className="w-12 h-12 text-[#8B7355]/40 mx-auto mb-3" />
          <p className="text-[#8B7355]">No kundli generated. Update client birth details to generate one.</p>
        </div>
      ) : null}
    </div>
  );
  ...
  if (user?.role !== 'astrologer' && user?.role !== 'admin') {
    return (
      <div className="max-w-4xl mx-auto py-24 px-4 text-center">
        <Star className="w-16 h-16 text-[#8B7355]/40 mx-auto mb-4" />
        <h2 className="text-2xl font-display font-bold text-[#1a1a2e] mb-2">Astrologer Access Required</h2>
        <p className="text-[#8B7355]">This panel is only accessible to verified astrologers.</p>
      </div>
    );
  }
  ...
  return (
    <section className="max-w-6xl mx-auto py-24 px-4">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-display font-bold text-[#1a1a2e]">{t('astrologer.panel')}</h2>
          <p className="text-sm text-[#8B7355]">{t('astrologer.clients')}</p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <div className="relative flex-1">
              ...
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#8B7355] hover:text-[#1a1a2e]">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              <User className="w-16 h-16 text-[#8B7355]/30 mx-auto mb-4" />
              <p className="text-lg text-[#8B7355] mb-1">{t('astrologer.noClients')}</p>
              <p className="text-sm text-[#8B7355]/70 mb-4">Add your first client to get started</p>
              <Button onClick={() => setView('add')}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {clients.map((client) => (
                <Card key={client.id}
                  className="bg-[#E8E0D4] border-0 shadow-sm hover:shadow-md transition-shadow cursor-pointer group"
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h4 className="font-display font-semibold text-[#1a1a2e] group-hover:text-[#B8860B] transition-colors">
                            {client.client_name}
                          </h4>
                          {client.client_phone && (
                      ...
                      <ChevronRight className="w-4 h-4 text-[#8B7355]/50 group-hover:text-[#B8860B] transition-colors mt-1" />
                    </div>
                    <div className="text-xs text-[#8B7355] space-y-1">
                      ...
                      <p className="text-[#8B7355]/50 mt-2">
