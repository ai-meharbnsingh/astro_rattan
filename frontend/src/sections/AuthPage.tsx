      ...
      <div className="w-full max-w-md mx-4 sm:mx-auto">
        <div className="text-center mb-8">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-sacred-gold to-sacred-saffron flex items-center justify-center mx-auto mb-4 shadow-glow-gold">
            <Stars className="w-8 h-8 text-background" />
          </div>
        ...
        <Tabs defaultValue="login" className="w-full">
          <TabsList className="grid grid-cols-2 bg-card mb-6 border border-sacred-gold">
            <TabsTrigger value="login" className="data-[state=active]:bg-sacred-gold data-[state=active]:text-background text-muted-foreground">{t('auth.signIn')}</TabsTrigger>
            <TabsTrigger value="register" onClick={() => { setRegStep('email'); setOtp(['','','','','','']); setEmailToken(''); setError(''); setSuccess(''); }} className="data-[state=active]:bg-sacred-gold data-[state=active]:text-background text-muted-foreground">{t('auth.signUp')}</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="register" className="mt-0">
            ...
            {regStep === 'email' && (
              <div className="space-y-3 sm:space-y-4">
                <div className="text-center mb-2">
                  ...
                  <p className="text-sm text-muted-foreground">{t('auth.enterEmailToReceiveCode')}</p>
                </div>
                <div className="relative">
                  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-sacred-gold" />
            ...
            {regStep === 'otp' && (
              <div className="space-y-3 sm:space-y-4">
                <button onClick={() => { setRegStep('email'); setError(''); setSuccess(''); }} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-sacred-gold transition-colors">
                ...
                <div className="text-center">
                  {countdown > 0 ? (
                    <p className="text-sm text-foreground">{t('auth.resendIn')} {countdown}s</p>
                  ) : (
        ...
        {showForgotPassword && (
          <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center pt-[10vh] sm:pt-0 bg-background backdrop-blur-sm px-4" onClick={() => setShowForgotPassword(false)}>
            <div className="bg-background border border-sacred-gold rounded-2xl p-6 max-w-sm w-full space-y-4" onClick={e => e.stopPropagation()}>
              ...
              {resetStep === 'otp' && (
                <>
                  <p className="text-sm text-foreground text-center">{t('auth.enterOtp')} {resetEmail}</p>
              ...
              <button onClick={() => setShowForgotPassword(false)} className="w-full text-center text-sm text-foreground hover:text-foreground">{t('auth.cancel')}</button>
            </div>
          </div>
        )}
      </div>
