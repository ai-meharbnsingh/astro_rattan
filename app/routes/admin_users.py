...
def deactivate_user(
    ...
    updated = db.execute(
        "SELECT id, email, name, role, is_active, updated_at FROM users WHERE id = ?",
        (user_id,),
    ).fetchone()
    return dict(updated)
...
def get_user_activity(
    ...
    consultations = db.execute(
        "SELECT id, type, status, scheduled_at, created_at FROM consultations WHERE user_id = ? ORDER BY created_at DESC",
        (user_id,),
    ).fetchall()

    ai_chats = db.execute(
        "SELECT id, chat_type, user_message, created_at FROM ai_chat_logs WHERE user_id = ? ORDER BY created_at DESC",
        (user_id,),
    ).fetchall()
