...
## Corpus Check
- 219 files · ~2,561,277 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2004 nodes · 3060 edges · 73 communities detected
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 304 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `KundliRequest` - 39 edges
2. `KundliMatchRequest` - 39 edges
3. `DivisionalChartRequest` - 39 edges
4. `analyze_yogas_and_doshas()` - 36 edges
5. `_fetch_kundli()` - 28 edges
6. `_h()` - 25 edges
7. `_chart_data()` - 23 edges
...
Cohesion: 0.04
Nodes (107): get_live_dashboard(), get_stats(), get_user_detail(), list_all_kundlis(), list_users(), Admin routes — user management, stats, kundli overview, live traffic panel., Change a user's role (user/astrologer/admin)., Activate or deactivate a user account. (+99 more)
...
Cohesion: 0.07
Nodes (78): _build_kundli_pdf(), _chart_data(), check_doshas(), _compute_dasha(), delete_all_my_kundlis(), delete_kundli(), download_kundli_pdf(), _fetch_kundli() (+70 more)
...
## Knowledge Gaps
- **602 isolated node(s):** `ai_engine.py — AI-Powered Vedic Astrology Interpretation Engine ================`, `Detect which AI provider to use. Priority: explicit > gemini > openai.`, `Lazy provider detection — re-reads config each time until a provider is found.`, `Call Google Gemini API via raw HTTP (no SDK dependency).`, `Call OpenAI chat completion API via raw HTTP (no SDK dependency).` (+597 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 58`** (2 nodes): `index.ts`, `Vercel serverless entry point — exposes the FastAPI app.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 59`** (2 nodes): `__init__.py`, `Route registry — import all routers for inclusion in the FastAPI app.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 60`** (2 nodes): `data.py`, `vastu/data.py — Complete Vastu Shastra Reference Data ==========================`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 61`** (2 nodes): `connection.js`, `testConnection()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 62`** (2 nodes): `predictions.js`, `auth()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 63`** (2 nodes): `remedies.js`, `auth()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 64`** (2 nodes): `KundliChart.jsx`, `KundliChart()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 65`** (1 nodes): `stress_test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 66`** (1 nodes): `ui-test.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 67`** (1 nodes): `tailwind.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 68`** (1 nodes): `playwright.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 71`** (1 nodes): `server.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 72`** (1 nodes): `aspect-ratio.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `KundliRequest` connect `Community 5` to `Community 2`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `KundliMatchRequest` connect `Community 5` to `Community 2`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `DivisionalChartRequest` connect `Community 5` to `Community 2`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Are the 37 inferred relationships involving `KundliRequest` (e.g. with `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,` and `Geocode a place name using the free Nominatim OpenStreetMap API.`) actually correct?**
  _`KundliRequest` has 37 INFERRED edges - model-reasoned connections that need verification._
- **Are the 37 inferred relationships involving `KundliMatchRequest` (e.g. with `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,` and `Geocode a place name using the free Nominatim OpenStreetMap API.`) actually correct?**
  _`KundliMatchRequest` has 37 INFERRED edges - model-reasoned connections that need verification._
- **Are the 37 inferred relationships involving `DivisionalChartRequest` (e.g. with `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,` and `Geocode a place name using the free Nominatim OpenStreetMap API.`) actually correct?**
  _`DivisionalChartRequest` has 37 INFERRED edges - model-reasoned connections that need verification._
