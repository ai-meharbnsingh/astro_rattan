...
## Corpus Check
- 560 files · ~5,643,280 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8774 nodes · 15395 edges · 299 communities detected
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 921 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `KundliRequest` - 83 edges
2. `KundliMatchRequest` - 83 edges
3. `DivisionalChartRequest` - 83 edges
4. `BirthRectificationRequest` - 83 edges
5. `SarvatobhadraRequest` - 83 edges
6. `D108AnalysisRequest` - 83 edges
7. `KPHoraryRequest` - 74 edges
8. `KPHoraryPredictRequest` - 74 edges
9. `PrashnaQuickRequest` - 74 edges
10. `M()` - 74 edges
...
Cohesion: 0.02
Nodes (169): get_live_dashboard(), get_stats(), get_user_detail(), list_all_kundlis(), list_users(), Admin routes — user management, stats, kundli overview, live traffic panel., Change a user's role (user/astrologer/admin)., Activate or deactivate a user account. (+161 more)

### Community 3 - "Community 3"
Cohesion: 0.05
Nodes (180): ashtottari_dasha(), moola_dasha(), Dasha routes — Ashtottari, Moola, and Tara dasha systems., Calculate Tara Dasha periods from birth nakshatra.      Divides 27 nakshatras in, Calculate Ashtottari Dasha periods from birth nakshatra.      The Ashtottari sys, Calculate Moola (Jaimini) Dasha periods.      Sign-based dasha system. Starting, tara_dasha(), birth_rectification() (+172 more)
...
Cohesion: 0.04
Nodes (148): add_chandra_journal(), add_family_link(), _build_sapt_var_entry(), checkin_remedy_tracker(), correlate_palm_marks(), create_remedy_tracker(), _degrees_to_dms(), delete_remedy_tracker() (+140 more)
...
Cohesion: 0.04
Nodes (109): analyze_yogas_and_doshas(), check_abhibhava_yoga(), check_adhi_yoga(), check_amala_yoga(), check_anapha_yoga(), check_angarak_dosha(), check_apta_yoga(), check_arabha_yoga() (+101 more)
...
Cohesion: 0.02
Nodes (40): Tests for app.panchang_yogas -- Special Yoga Calculations ======================, Sunday + Rohini → inactive (needs Hasta)., Monday + Hasta → inactive (Monday needs Rohini)., Sunday (0) + Tritiya (3) + Chitra → active., Tuesday (2) + Ashtami (8) + Mrigashira → active., Saturday (6) + Trayodashi (13) + Vishakha → active., Monday (1) + Tritiya (3) + Chitra → inactive (Monday not valid)., Sunday (0) + Dwitiya (2) + Chitra → inactive (Dwitiya not in Dwipushkar tithis). (+32 more)
...
Cohesion: 0.04
Nodes (88): analyze_loshu_arrows(), analyze_loshu_planes(), analyze_missing_numbers(), analyze_name_numerology(), analyze_repeated_numbers(), _birthday_number(), _build_number_affinities(), _build_pair_combination_table() (+80 more)
...
Cohesion: 0.04
Nodes (81): _approx_sunrise_sunset(), calculate_abhijit_muhurat(), calculate_brahma_muhurat(), calculate_chandrashtama(), calculate_choghadiya(), calculate_gulika_kaal(), calculate_night_choghadiya(), calculate_panchang() (+73 more)
...
Cohesion: 0.04
Nodes (64): assemble_section(), calculate_transit_forecast(), calculate_transit_houses(), calculate_transits(), _check_sade_sati(), _compute_ashtak_adjustment(), compute_scores(), _compute_tithi() (+56 more)
...
Cohesion: 0.07
Nodes (68): analyze_apatya(), _aspects_house(), _assess_prospect(), _build_fifth_house_analysis(), _children_timing_section(), _detect_adoption_indicators(), _detect_aputra_yoga(), _detect_bahu_putra_yoga() (+60 more)
...
Cohesion: 0.1
Nodes (60): build_endpoint_calls(), _call(), _classify(), code_block(), collect_all(), create_kundli(), EndpointResult, engine_confidence() (+52 more)
...
Cohesion: 0.03
Nodes (28): app_client(), Route-level tests for the 7 new API endpoints.  Tests are split into two parts:, Verify KP Horary engine via route handler arguments., Verify KP Horary prediction engine via route handler arguments., Verify Birth Rectification engine via route handler arguments., Verify Sarvatobhadra Chakra engine via route handler arguments., Verify D108 analysis engine via route handler arguments., Verify Ashtottari Dasha engine via route handler arguments. (+20 more)
...
Cohesion: 0.03
Nodes (23): tests/test_lalkitab_time_planet.py  Unit tests for app/lalkitab_time_planet.py —, When sunrise_hms is None but fallback is allowed, sunrise_assumed=True., With 06:00 fallback, hora IS computed → hora_skipped=False., The 06:00 fallback computes a real hora lord., Without hora_lord, dual cannot be True., Without hora_lord, doubled cannot be True., Find a date/time combination where day_lord != hora_lord.         Sunday (Sun) a, When dual=True, both_planets should list both day_lord and hora_lord. (+15 more)

### Community 35 - "Community 35"
Cohesion: 0.04
Nodes (25): app_client(), _make_mock_db(), mock_db_dep(), Tests for the 4 new kundli GET routes:   - GET /{kundli_id}/gochara-vedha   - GE, Verify TRANSIT_FRAGMENTS structure is correct., Verify transit_lucky.get_all_lucky_metadata with known inputs., Return a mock DB whose execute().fetchone() returns the mock row., Module-scoped TestClient with auth override and per-request DB override. (+17 more)

### Community 36 - "Community 36"
Cohesion: 0.07
Nodes (42): _approx_sunrise_sunset(), _check_dynamic_output(), _classify_output(), _compute_independent_longitudes(), _engine_muhurat(), _engine_panchang(), generate_report(), _get() (+34 more)

### Community 37 - "Community 37"
Cohesion: 0.05
Nodes (56): _abda_bala(), _angular_distance(), _aspect_strength(), _ayana_bala(), calculate_bhav_bala(), calculate_shadbala(), _chandrastha_bala(), _chandravritta_bala() (+48 more)
...
Cohesion: 0.04
Nodes (23): tests/test_lalkitab_contract_bugs.py ==================================== TDD te, None is a plausible 'not available' value — keep existing graceful degradation., Empty string should degrade gracefully (already correct)., Meharban Singh chart: Taurus asc + Rahu in H1 → 36-Sala., build_prediction_studio() must raise TypeError when planet_positions is not a di, BUG-4: list passed as planet_positions must raise TypeError, not AttributeError., None as planet_positions must raise TypeError., string as planet_positions must raise TypeError. (+15 more)
...
Cohesion: 0.07
Nodes (23): calcLifePath(), calcPersonalDay(), compactLine(), extractStr(), formatTime12(), formatTime24(), generateObservancesFromDay(), getLocalDateString() (+15 more)
...
Cohesion: 0.05
Nodes (11): Smoke tests for critical engine modules: varshphal, transit, astro_iogita, dasha, Smoke tests for transit_engine.py., Smoke tests for astro_iogita_engine.py., Smoke tests for dasha_engine.py., One full 120-year cycle = exactly 120 years total., When moon_longitude is given, first dasha balance is partial.         One 120-ye, Smoke tests for varshphal_engine.py., TestAstroIogitaEngine (+3 more)

### Community 60 - "Community 60"
Cohesion: 0.09
Nodes (39): _approx_ascendant(), _approx_ayanamsa(), _approx_moon_longitude(), _approx_planet_longitude(), _approx_rahu_longitude(), _approx_sun_longitude(), _build_status(), _calculate_fallback() (+31 more)
...
Cohesion: 0.06
Nodes (33): Tests for numerology_engine.py — Pythagorean numerology calculations., Day 15 should have a different title than day 6 (both reduce to 6)., Every birthday prediction must have title_hi and talent_hi., calculate_numerology should return birthday_reduced as reduced digit., MATURITY_PREDICTIONS must have description and advice for all entries., Verify master number 11 is preserved (not reduced to 2)., calculate_numerology maturity_prediction should include description., VEHICLE_PREDICTIONS should have energy_hi and prediction_hi for all entries. (+25 more)
...
Cohesion: 0.14
Nodes (29): get_all_signs_horoscope(), get_daily_horoscope(), get_monthly_horoscope(), get_natal_sign(), get_tomorrow_horoscope(), get_transit_insights(), get_weekly_horoscope(), get_yearly_horoscope() (+21 more)
...
Cohesion: 0.08
Nodes (14): chk(), en(), erow(), fmt(), g(), get_server_version(), hi(), Fetch server version from /api/health or /api/version. Returns 'unknown' if not (+6 more)
...
Cohesion: 0.17
Nodes (18): _birth_qs(), _engine_birth_chart(), _engine_dasha(), _extract_transit_claims(), _get(), _house_from_lagna(), main(), _overlap_label() (+10 more)
...
Cohesion: 0.15
Nodes (13): err(), h1(), h2(), h3(), h4(), hr(), nl(), Safely extract data from a key. Returns None if ok=False or missing. (+5 more)
...
Cohesion: 0.17
Nodes (17): _build_pair_index(), _calc_effect_strength(), _check_d12_conjunction(), _degree_separation(), detect_conjunctions(), _house(), _is_debilitated(), load_conjunction_data() (+9 more)
...
Cohesion: 0.12
Nodes (17): list_activities(), _monthly_days(), muhurat_find(), muhurat_finder(), muhurat_monthly(), Muhurat Finder API — activity-specific auspicious date finder + compatibility ro, Daily window compatibility endpoint for a selected date., ⚠ DEPRECATED: Use /api/muhurat/finder instead.      Daily window compatibility e (+9 more)
...
Cohesion: 0.18
Nodes (12): _check_common_fields(), _positions(), tests/test_lalkitab_rahu_ketu_axis.py Tests for detect_rahu_ketu_axis() in app/l, Build a minimal planet_positions list with Rahu and Ketu., test_all_six_axes_return_unique_axis_keys(), test_houses_not_6_apart_returns_none(), test_is_symmetric_always_true(), test_life_areas_non_empty_for_all_axes() (+4 more)
...
Cohesion: 0.17
Nodes (15): generate_daily_horoscopes(), generate_horoscope(), _generate_template_horoscope(), _get_current_transits(), _get_current_transits_full(), H-09: Horoscope Content Generation Pipeline — seeds daily & weekly horoscopes., Calculate today's planetary positions (signs) for horoscope weighting., Get full planetary data for transit calculations (not just sign names). (+7 more)
...
Cohesion: 0.17
Nodes (9): _check_common_fields(), tests/test_lalkitab_chakar.py Tests for detect_chakar_cycle() in app/lalkitab_ch, Assert every result carries the mandatory invariant fields., test_aquarius_rahu_in_h1_36_sala(), test_cancer_ketu_in_h1_36_sala(), test_leo_ascendant_sun_lord_35_sala(), test_rahu_ketu_both_in_h1_rahu_wins(), test_scorpio_ascendant_mars_lord_35_sala() (+1 more)
...
Cohesion: 0.15
Nodes (7): dump(), j(), Call fn(*args, **kwargs), return (result, error_str). Never crashes., Render a markdown table. headers=list, rows=list of lists., run(), status_block(), table()
...
Cohesion: 0.24
Nodes (13): _build_prompt(), _call_ollama(), generate_slot(), _gochara_tone(), _house_significations(), load_progress(), main(), _parse_response() (+5 more)

### Community 149 - "Community 149"
Cohesion: 0.15
Nodes (6): dump(), j(), Call fn(*args, **kwargs), return (result, error_str). Never crashes., Render a markdown table., run(), table()
...
Cohesion: 0.15
Nodes (1): Numerology engine core number tests — 5 manually verified numbers. DOB: 1985-08-
...
Cohesion: 0.23
Nodes (11): _core_pattern(), _dasha_2yr_outlook(), generate_master_summary(), _main_problem(), lalkitab_master_summary.py — Master Summary Generator for Lal Kitab charts.  All, Identify the single most pressing problem: planet with highest urgency     and l, Build a 2-year narrative from actual saala grah dasha data.     Returns: timelin, Generate a complete master summary for a Lal Kitab chart.      Args:         pos (+3 more)
...
Cohesion: 0.23
Nodes (11): database_seed_lalkitab.py — Idempotent seed data for Lal Kitab DB tables.  Table, Patch urdu_latin into existing lk_farmaan rows using _FARMAAN_URDU_LATIN., Idempotent seed of all three Lal Kitab DB tables.     Uses ON CONFLICT DO NOTHIN, Seed nishaniyan_master. Handles both 5-column and 6-column (with nishani_text_en, Seed lal_kitab_debts. DB columns: debt_type, planet, description, indication, re, Seed lk_interpretations table (created by migration if not yet present)., _seed_debts(), _seed_farmaan() (+3 more)
...
Cohesion: 0.26
Nodes (11): _calc_age(), get_dasha_timeline(), get_saala_grah(), _parse_date(), _planet_at_age(), lalkitab_dasha.py — Lal Kitab Saala Grah & 35-Year Dasha System ================, Return the Saala Grah planet name for a given age (1-based)., Return completed years of age (integer floor). (+3 more)
...
Cohesion: 0.44
Nodes (10): auth_headers(), check(), login(), Login and return JWT token. Raises on failure., run_horoscope(), run_kundli(), run_lalkitab(), run_numerology() (+2 more)

### Community 166 - "Community 166"
Cohesion: 0.25
Nodes (10): _build_interpretation(), _build_mitigating_factors(), calculate_upagrahas(), _compute_house(), _planet_aspects_house_u(), Compute mitigating factors for an upagraha in `house`.      Returns:         (fa, Return (en, hi) interpretation for an upagraha in a given house., Calculate Upagrahas (sub-planets) including Aprakasha Grahas and Kala Velas. (+2 more)
...
Cohesion: 0.22
Nodes (1): Edge-case DOB tests: master number Life Path preservation (11, 22, 33). Verifies
...
Cohesion: 0.29
Nodes (7): calculate_age_milestones(), get_seven_year_cycle(), _parse_date(), lalkitab_milestones.py — Safar-e-Zindagi (Age Milestone Triggers) ==============, Parse YYYY-MM-DD or DD/MM/YYYY — raises ValueError for invalid strings., birth_date: "YYYY-MM-DD"     planet_positions: [{"planet": "Saturn", "house": 8}, Returns the active 7-year sub-cycle, adjacent cycles, and ruler quality for curr
...
Cohesion: 0.5
Nodes (7): bar(), clear(), eta_str(), load_progress(), main(), planet_grid(), tail_log()
...
Cohesion: 0.38
Nodes (6): build_rules(), compute_lk_strength(), get_lk_strength_detail(), Lal Kitab Rules Engine (backend)  Purpose: - Provide rule-driven structures used, Return (score 0-100, label) using LK 1952 Pakka Ghar system.     This replaces R, Full strength breakdown for a planet in its LK house.
...
Cohesion: 0.43
Nodes (5): _current_age(), generate_insights(), _pick_current_pinnacle(), numerology_insight_engine.py — Master Insight Layer ============================, Generate the master insight block from a calculate_numerology() result.      Arg
...
Cohesion: 0.4
Nodes (5): calculate_mook_prashna(), compute_mook_number(), numerology_mook_prashna_engine.py — Mook Prashna & Khoyi Vastu =================, Sum the 9 numbers written by the person and add 3.     Result is always in range, Given 9 numbers, compute the mook prashna result.      Returns:         dict wit
...
Cohesion: 0.5
Nodes (3): calculate_nadi_insights(), nadi_engine.py -- Nadi Astrology Interpretive Engine ===========================, Generate Nadi astrology insights from planet placements.     Covers: same-house
...
Cohesion: 0.67
Nodes (3): calculate_yogini_dasha(), get_starting_yogini(), Calculate Yogini Dasha periods covering 120 years (multiple 36-year cycles).

### Community 215 - "Community 215"
Cohesion: 0.5
Nodes (3): analyze_marriage(), lalkitab_marriage.py — Lal Kitab Marriage & Relationship Analysis ==============, Analyze marriage prospects from Lal Kitab chart positions.      Args:         po
...
Cohesion: 0.5
Nodes (3): get_personalized_tasks(), Chandra Chalana 43-day protocol tasks (backend).  The protocol itself is tracked, Return the 43 Chandra Chalana tasks with milestone-day overrides for the given M
...
Cohesion: 1.0
Nodes (1): Auto-generated transit interpretation variants.  Generated by scripts/generate_t
...
## Knowledge Gaps
- **2112 isolated node(s):** `Safely extract data from a key. Returns None if ok=False or missing.`, `Return error message for a failed key.`, `Return STATUS string for a key.`, `Lal Kitab remedy context — Sun, Moon, Mars (36 entries)`, `Tests for enhanced Choghadiya — Vaar Vela, Kaal Vela, Kaal Ratri flags.` (+2107 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 226`** (2 nodes): `scratch_check_i18n.py`, `check_file()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (2 nodes): `scratch_check_backend.py`, `check_file()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (2 nodes): `lk_ctx_a.py`, `Lal Kitab remedy context — Sun, Moon, Mars (36 entries)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (2 nodes): `lk_ctx_c.py`, `Lal Kitab remedy context — Saturn, Rahu, Ketu (36 entries)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (2 nodes): `lk_ctx_b.py`, `Lal Kitab remedy context — Mercury, Jupiter, Venus (36 entries)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (2 nodes): `test_fest_engine.py`, `test_festivals()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (2 nodes): `test_flags.py`, `test_sunrise_flags()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (2 nodes): `__init__.py`, `Route registry — import all routers for inclusion in the FastAPI app.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (2 nodes): `transit_variants.py`, `Auto-generated transit interpretation variants.  Generated by scripts/generate_t`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (2 nodes): `lalkitab_translations.py`, `lalkitab_translations.py — Lal Kitab bilingual constants =======================`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (2 nodes): `lalkitab_remedy_context.py`, `Lal Kitab 1952 — Remedy Context: problem / reason / how_it_works Source: Pt. Roo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (2 nodes): `transit_interpretations.py`, `Transit interpretation fragments — 9 planets x 12 houses x 5 areas, bilingual.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (2 nodes): `data.py`, `vastu/data.py — Complete Vastu Shastra Reference Data ==========================`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (2 nodes): `analyze-i18n.js`, `walk()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (2 nodes): `BeneficMaleficLegend.tsx`, `BeneficMaleficLegend()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (2 nodes): `test_transit_interpretations_completeness.py`, `test_transit_fragments_include_9_planets_and_full_matrix()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (2 nodes): `test_panchang_sankranti_route.py`, `test_panchang_sankranti_route_returns_12()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (2 nodes): `connection.js`, `testConnection()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (2 nodes): `predictions.js`, `auth()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (2 nodes): `remedies.js`, `auth()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (2 nodes): `KundliChart.jsx`, `KundliChart()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (1 nodes): `check_swe_help.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `test_gemini.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 249`** (1 nodes): `stress_test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `Krishna Pratipada to Panchami (16-20) normalise to 1-5 → Full Life.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `Krishna Shashthi to Dashami (21-25) normalise to 6-10 → Half Life.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): `Krishna Ekadashi to Chaturdashi (26-29) normalise to 11-14 → Weak Life.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 253`** (1 nodes): `ui-test.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (1 nodes): `tailwind.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 255`** (1 nodes): `playwright.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 256`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `postcss.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 258`** (1 nodes): `palette-CXiXSgC5.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 259`** (1 nodes): `house-DtiC-4UK.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 260`** (1 nodes): `hash-CicP2k4H.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `sun-BzBhZV-E.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 262`** (1 nodes): `zap-CAEtoi2h.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `circle-alert-C7OzIryt.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 264`** (1 nodes): `circle-check-big-C2G1rGUt.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (1 nodes): `plus-0BNEFBy0.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `activity-X4r3g2MZ.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 267`** (1 nodes): `info-CrU8AAHX.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `chevron-left-De_SGD7e.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 269`** (1 nodes): `search-B7V9CMzU.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 270`** (1 nodes): `chevron-up-oUmfnrAD.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 271`** (1 nodes): `navigation-KikWBpYy.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 272`** (1 nodes): `arrow-left-B1R7J2oO.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 273`** (1 nodes): `save-CurQzcRD.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 274`** (1 nodes): `circle-check-CgXHvHNa.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 275`** (1 nodes): `calendar-BAWu2H0T.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 276`** (1 nodes): `eye-CP953dOK.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 277`** (1 nodes): `map-pin-4ZjEbj4U.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 278`** (1 nodes): `orbit-BJqvfdpJ.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 279`** (1 nodes): `briefcase-CKH9P3l8.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 280`** (1 nodes): `rotate-ccw-DKeW6YZS.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 281`** (1 nodes): `clock-BmMvehyB.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 282`** (1 nodes): `chevron-right-BX36YS4o.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 283`** (1 nodes): `send-DBV8maWO.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 284`** (1 nodes): `trending-up-D6lZAkWG.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 285`** (1 nodes): `triangle-alert-Djnuq5R0.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 286`** (1 nodes): `book-open-DsqoyRev.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 287`** (1 nodes): `loader-circle-DHqOI4NW.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (1 nodes): `wallet-Wc7AxNdH.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 289`** (1 nodes): `shield-check-CeboXS6O.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 290`** (1 nodes): `sticky-note-Dh-n4DfT.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 291`** (1 nodes): `arrow-right-Dg-Z4lpq.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 292`** (1 nodes): `index.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `Sample parts 0..107 for each sign and confirm all 12 target signs appear.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 294`** (1 nodes): `Each planet must be in the exact sign+house from the ref snapshot.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 295`** (1 nodes): `wxLogin.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `server.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `aspect-ratio.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `test_fallback.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `js()` connect `Community 4` to `Community 41`, `Community 13`, `Community 7`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `KPHoraryRequest` connect `Community 6` to `Community 2`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Why does `KPHoraryPredictRequest` connect `Community 6` to `Community 2`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Are the 81 inferred relationships involving `KundliRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`KundliRequest` has 81 INFERRED edges - model-reasoned connections that need verification._
- **Are the 81 inferred relationships involving `KundliMatchRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`KundliMatchRequest` has 81 INFERRED edges - model-reasoned connections that need verification._
- **Are the 81 inferred relationships involving `DivisionalChartRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`DivisionalChartRequest` has 81 INFERRED edges - model-reasoned connections that need verification._
- **Are the 81 inferred relationships involving `BirthRectificationRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`BirthRectificationRequest` has 81 INFERRED edges - model-reasoned connections that need verification._