...
## Corpus Check
- 478 files · ~5,102,421 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7477 nodes · 13021 edges · 247 communities detected
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 733 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `M()` - 74 edges
2. `KundliRequest` - 69 edges
3. `KundliMatchRequest` - 69 edges
4. `DivisionalChartRequest` - 69 edges
5. `BirthRectificationRequest` - 69 edges
6. `SarvatobhadraRequest` - 69 edges
7. `D108AnalysisRequest` - 69 edges
8. `KPHoraryRequest` - 60 edges
9. `KPHoraryPredictRequest` - 60 edges
10. `o()` - 56 edges
...
Cohesion: 0.01
Nodes (123): apiFetch(), fetchWithRetry(), friendlyError(), tryRefreshToken(), rotateByLagna(), SAVKundliChart(), handleOtpKeyDown(), handleResendOtp() (+115 more)
...
Cohesion: 0.03
Nodes (154): get_live_dashboard(), get_stats(), get_user_detail(), list_all_kundlis(), list_users(), Admin routes — user management, stats, kundli overview, live traffic panel., Change a user's role (user/astrologer/admin)., Activate or deactivate a user account. (+146 more)
...
Cohesion: 0.06
Nodes (142): birth_rectification(), _build_kundli_pdf(), _chart_data(), check_doshas(), _compute_dasha(), create_download_token(), d108_analysis(), delete_all_my_kundlis() (+134 more)
...
Cohesion: 0.04
Nodes (115): add_chandra_journal(), add_family_link(), checkin_remedy_tracker(), correlate_palm_marks(), create_remedy_tracker(), delete_remedy_tracker(), delete_saved_prediction(), _derive_lk_house() (+107 more)
...
Cohesion: 0.07
Nodes (33): calculate_achanak_chot(), calculate_bunyaad(), calculate_dhoka(), calculate_enemy_presence(), calculate_hora_lord(), calculate_karmic_debts(), calculate_karmic_debts_with_hora(), calculate_kayam_grah() (+25 more)
...
Cohesion: 0.11
Nodes (31): analyze_antardasha_phala(), analyze_mahadasha_phala(), _assess_planet_strength(), _build_antardasha_periods(), _build_pratyantar_periods(), calculate_dasha(), _calculate_dasha_balance(), calculate_extended_dasha() (+23 more)
...
Cohesion: 0.14
Nodes (30): analyze_bhava_vichara(), _analyze_karaka_as_lagna(), _asc_sign(), _aspects_on_house(), _assess_bhava(), _bhava_karakas(), _house_lord(), _house_sign() (+22 more)
...
Cohesion: 0.12
Nodes (28): analyze_vritti(), _ascendant_sign(), _dignity_score(), _empty_result(), _house_score(), load_vritti_data(), _lord_of_tenth(), _navamsa_sign() (+20 more)
...
Cohesion: 0.11
Nodes (27): analyze_ashtakvarga_phala(), _apply_ekadhipatya_shodhana(), _apply_trikona_shodhana(), calculate_ashtakvarga(), _calculate_shodhya_pinda(), _combo_effect(), _house_number_from_sign(), _house_status_from_bindus() (+19 more)
...
Cohesion: 0.16
Nodes (25): analyze_longevity_indicators(), _asc_sign(), _eighth_house_section(), _int_house(), _is_debilitated(), _is_exalted(), _is_own(), _karmic_transitions_narrative() (+17 more)
...
Cohesion: 0.13
Nodes (21): _add_years(), _assign_deha_jeeva(), _build_sign_sequence(), _calculate_balance(), calculate_kalachakra_dasha(), _find_deha_jeeva_rashis(), _is_savya(), _moon_nakshatra_pada() (+13 more)
...
Cohesion: 0.18
Nodes (15): build_prediction_studio(), _build_specific_text(), compute_area_score(), _dignity_phrase(), _dignity_phrase_hi(), _house_strength(), _navamsa_dignity_adjustment(), PredictionArea (+7 more)
...
Cohesion: 0.18
Nodes (11): check_day_favorable(), get_activity_info(), get_activity_rules(), get_all_activities(), normalize_tithi_for_rules(), Muhurat rules database for 9 activity-specific muhurats.  Each activity defines, Get activity name, Hindi name, icon, description., Get the full rule set for an activity. (+3 more)
...
Cohesion: 0.5
Nodes (3): find_muhurat_dates(), Muhurat Finder Engine — finds auspicious dates for specific activities.  Classic, Find auspicious dates for an activity in a given month.      Optional birth_moon
...
## Knowledge Gaps
- **1765 isolated node(s):** `Lal Kitab remedy context — Sun, Moon, Mars (36 entries)`, `Tests for enhanced Choghadiya — Vaar Vela, Kaal Vela, Kaal Ratri flags.`, `Exactly ONE day period must have vaar_vela=True for each weekday.`, `Exactly ONE day period must have kaal_vela=True for each weekday.`, `Exactly ONE night period must have kaal_ratri=True for each weekday.` (+1760 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 180`** (2 nodes): `lk_ctx_a.py`, `Lal Kitab remedy context — Sun, Moon, Mars (36 entries)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 181`** (2 nodes): `lk_ctx_c.py`, `Lal Kitab remedy context — Saturn, Rahu, Ketu (36 entries)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (2 nodes): `lk_ctx_b.py`, `Lal Kitab remedy context — Mercury, Jupiter, Venus (36 entries)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (2 nodes): `test_fest_engine.py`, `test_festivals()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 184`** (2 nodes): `test_flags.py`, `test_sunrise_flags()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (2 nodes): `__init__.py`, `Route registry — import all routers for inclusion in the FastAPI app.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (2 nodes): `lalkitab_translations.py`, `lalkitab_translations.py — Lal Kitab bilingual constants =======================`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (2 nodes): `lalkitab_chandra_tasks.py`, `Chandra Chalana 43-day protocol tasks (backend).  The protocol itself is tracked`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 188`** (2 nodes): `lalkitab_remedy_context.py`, `Lal Kitab 1952 — Remedy Context: problem / reason / how_it_works Source: Pt. Roo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (2 nodes): `transit_interpretations.py`, `Transit interpretation fragments — 9 planets x 12 houses x 5 areas, bilingual.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (2 nodes): `data.py`, `vastu/data.py — Complete Vastu Shastra Reference Data ==========================`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (2 nodes): `analyze-i18n.js`, `walk()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (2 nodes): `test_transit_interpretations_completeness.py`, `test_transit_fragments_include_9_planets_and_full_matrix()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (2 nodes): `connection.js`, `testConnection()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (2 nodes): `predictions.js`, `auth()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (2 nodes): `remedies.js`, `auth()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (2 nodes): `KundliChart.jsx`, `KundliChart()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `check_swe_help.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `stress_test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `Krishna Pratipada to Panchami (16-20) normalise to 1-5 → Full Life.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (1 nodes): `Krishna Shashthi to Dashami (21-25) normalise to 6-10 → Half Life.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Krishna Ekadashi to Chaturdashi (26-29) normalise to 11-14 → Weak Life.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `ui-test.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `tailwind.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `playwright.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `postcss.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `palette-CXiXSgC5.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `house-DtiC-4UK.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `hash-CicP2k4H.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `sun-BzBhZV-E.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `zap-CAEtoi2h.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `circle-alert-C7OzIryt.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `circle-check-big-C2G1rGUt.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `plus-0BNEFBy0.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `activity-X4r3g2MZ.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `info-CrU8AAHX.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `chevron-left-De_SGD7e.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `search-B7V9CMzU.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `chevron-up-oUmfnrAD.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `navigation-KikWBpYy.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `arrow-left-B1R7J2oO.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `save-CurQzcRD.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `circle-check-CgXHvHNa.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `calendar-BAWu2H0T.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `eye-CP953dOK.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `map-pin-4ZjEbj4U.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `orbit-BJqvfdpJ.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `briefcase-CKH9P3l8.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `rotate-ccw-DKeW6YZS.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `clock-BmMvehyB.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `chevron-right-BX36YS4o.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `send-DBV8maWO.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `trending-up-D6lZAkWG.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `triangle-alert-Djnuq5R0.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `book-open-DsqoyRev.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `loader-circle-DHqOI4NW.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `wallet-Wc7AxNdH.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `shield-check-CeboXS6O.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `sticky-note-Dh-n4DfT.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `arrow-right-Dg-Z4lpq.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `index.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `Sample parts 0..107 for each sign and confirm all 12 target signs appear.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `Each planet must be in the exact sign+house from the ref snapshot.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (1 nodes): `wxLogin.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `server.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (1 nodes): `aspect-ratio.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `js()` connect `Community 3` to `Community 34`, `Community 12`, `Community 5`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
- **Why does `KPHoraryRequest` connect `Community 9` to `Community 2`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Why does `KPHoraryPredictRequest` connect `Community 9` to `Community 2`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Are the 67 inferred relationships involving `KundliRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`KundliRequest` has 67 INFERRED edges - model-reasoned connections that need verification._
- **Are the 67 inferred relationships involving `KundliMatchRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`KundliMatchRequest` has 67 INFERRED edges - model-reasoned connections that need verification._
- **Are the 67 inferred relationships involving `DivisionalChartRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`DivisionalChartRequest` has 67 INFERRED edges - model-reasoned connections that need verification._
- **Are the 67 inferred relationships involving `BirthRectificationRequest` (e.g. with `FreePreviewRequest` and `Kundli routes — generate, retrieve, list, iogita analysis, match, dosha, dasha,`) actually correct?**
  _`BirthRectificationRequest` has 67 INFERRED edges - model-reasoned connections that need verification._